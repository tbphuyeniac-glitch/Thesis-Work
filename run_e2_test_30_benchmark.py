"""
E2 benchmark on the held-out 30-test-scenario set.

Picks representative small / medium / large scenarios from the test_baselines
tarball (already extracted under Test 30 scenarios/test_baselines/), runs A0
(linear pricing under SLA penalty, no B&P) and E2_pruned_gnn (pruned-exact
MIQP + GNN ranking, no B&P) on each, and records per-variant metrics.

Each scenario is a pre-shocked IRPData + ALNS baseline solution, so we skip
the (~120s) baseline-ALNS solve and call `run_lt_recourse_from_baseline`
directly. SLA calibration matches what the teacher was generated with:
mu = nu = 0.005, alpha = 4, beta = 2.
"""
...
# Calibration: stronger SLA penalty (μ ×20, ν ×5) — see if making penalty
# matter more in objective changes E2's per-size win rate. GNN was trained at
# μ=ν=0.005 so signal is mismatched but worth measuring magnitude effect.
os.environ["IRP_SLA_PENALTY"] = "on"
os.environ["IRP_SLA_MU"] = os.environ.get("IRP_SLA_MU", "0.10")
os.environ["IRP_SLA_NU"] = os.environ.get("IRP_SLA_NU", "0.025")
...
# Run ALL 30 scenarios so per-size variance is averaged out and we can
# tell systematic E2-vs-A0 patterns from random shock-instance noise.
SCENARIO_GROUPS = {
...
def _run_variant(variant: str, scen: Dict[str, Any]) -> Dict[str, Any]:
    """Run one variant (A0 or E2) on one pre-shocked scenario."""
    ...
    # Pull metrics. results is the dict returned by run_lt_recourse_from_baseline.
    cg = results.get("cg_solution")
    rmp_obj = float(getattr(cg, "objective", float("nan"))) if cg else float("nan")
    # CG iter count: prefer cg_engine telemetry (one entry per pricing iter
    # at the root B&P node, since use_branch_and_price=False).
    ep_hist = results.get("cg_episode_history") or []
    cg_iters = len(ep_hist) if ep_hist else 0

    # Realized cost breakdown (with LT)
    ...
    return {
        ...
        "columns_generated": int(results.get("columns_generated") or results.get("n_columns_generated") or 0),
        "columns_added_to_rmp": int(results.get("columns_added_to_rmp") or results.get("n_columns_added_to_rmp") or 0),
    }
...
def main() -> int:
    ...
    out_json = OUT_DIR / "comparison_per_run.json"
