"""
Ablation: pricing alignment vs GNN ranking — which mechanism reduces CG iters?

Three variants on all 30 test scenarios at MATCHED training regime μ=ν=0.05:

  A0_LIN   = exact_full_mode + LINEAR pricing      (current A0)
  A0_QUAD  = exact_full_mode + QUADRATIC pricing   (NEW — isolates pricing-alignment effect)
  E2       = pruned_exact + GNN top-frac + QUAD    (current E2)

Per-iter logic:
  A0_LIN  → A0_QUAD : adds quadratic SLA terms to pricing MIP objective
                       (keeps "all neg-RC columns" selection)
  A0_QUAD → E2      : adds 4-feature pair-pruning + GNN ranking + top-fraction filter

Comparison:
  A0_LIN vs A0_QUAD → contribution of pricing-RMP cost alignment (mechanism #1)
  A0_QUAD vs E2     → contribution of GNN ranking + pair pruning (mechanism #2)
"""
...
# MATCHED regime — same as E2 teacher data generation
...
def main() -> int:
    ...
    for label, scen_name in PICKS:
        ...
        for variant in ("A0_LIN", "A0_QUAD", "E2"):
            print(f"\n[RUN] {label} / {variant}")
            try:
    ...
    out_json = OUT_DIR / "ablation_pricing_vs_gnn.json"
