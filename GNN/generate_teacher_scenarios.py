...
def main() -> None:
    ...
    parser.add_argument(
        "--baseline-cache-dir", default=None,
        help="Directory of pre-computed ALNS baseline pkl.gz files from "
             "generate_test_baselines.py.  If a matching {base_id}.pkl.gz "
             "exists it is loaded instead of re-running ALNS.",
    )
    ...
    for base_idx, base in enumerate(bases, start=1):
        ...
        try:
            # Load from pre-computed pkl cache if available (generated by
            # generate_test_baselines.py).  Falls back to re-solving ALNS.
            loaded_from_cache = False
            if args.baseline_cache_dir:
                import gzip as _gz, pickle as _pkl
                _pkl_path = Path(args.baseline_cache_dir) / f"{base_id}.pkl.gz"
                if _pkl_path.exists():
                    try:
                        with _gz.open(_pkl_path, "rb") as _f:
                            _cached = _pkl.load(_f)
                        base_cache[base_id] = (_cached["base_data"], _cached["baseline_sol"])
                        loaded_from_cache = True
                        print(
                            f"  [baseline {base_idx:02d}/{n_bases:02d}] {base_id}: "
                            f"loaded from cache ({_pkl_path.stat().st_size // 1024} KB)",
                            flush=True,
                        )
                    except Exception as _e:
                        print(
                            f"  [baseline cache] {base_id}: pkl load failed ({_e}), re-solving",
                            flush=True,
                        )
