...
# data_dic = parse_data_file('./new_instances/mirplr-5-1-1-4.dat')
...
for m in range(product_kind):
    for i in range(retailer_number):
        him_wijm_produm+=-beta[iter_cnt] * him[i][m]*(-dim_prime[i][m])
        for j in range(retailer_number):
            if i != j:
                him_wijm_produm.addTerms(beta[iter_cnt]  * him[i][m], wijm[i][j][m])
                him_wijm_produm.addTerms(-beta[iter_cnt] * him[i][m], wijm[j][i][m])
