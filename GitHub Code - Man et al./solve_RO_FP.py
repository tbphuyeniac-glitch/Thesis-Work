...
# data_dic = parse_data_file('./new_instances/mirplr-5-1-3-1.dat')
...
# for i in range(nodes_number):
#     for m in range(product_kind):
#         master_obj +=him[i][m] * Iim0[i][m]
#         if i < j:
#             for k in range(vehicle_number):
#                 master_obj.addTerms(him[i][m], qimk[i][m][k])
for k in range(vehicle_number):
    # print(k)
...
while ((master.ObjVal) > delta_a and iter_cnt < max_iter) or (iter_cnt == 0):
    ...
    # for i in range(nodes_number):
    #     for m in range(product_kind):
    #         # master_obj.addTerms(-alpha[iter_cnt] * him[i][m], Iim0[i][m])
    #         master_obj += -alpha[iter_cnt] * him[i][m] * Iim0[i][m]
    #         if i < j:
    #             for k in range(vehicle_number):
    #                 master_obj.addTerms(-alpha[iter_cnt] * him[i][m], qimk[i][m][k])
    for k in range(vehicle_number):
...
if alpha[iter_cnt] != 0:
    ...
    for i in range(retailer_number):
        for m in range(product_kind):
            for k in range(vehicle_number):
                if qimk[i][m][k].x > 0.1:
                    fenmu+=pim[i][m]*qimk[i][m][k].x
    for i in range(nodes_number):
        for j in range(nodes_number):
            if i < j:
                for k in range(vehicle_number):
                    fenzi+=cij[i][j]* xijk[i][j][k].x
    # 3rd revision
    for i in range(nodes_number):
        for m in range(product_kind):
            fenzi +=him[i][m] * Iim0[i][m]
            if i < j:
                for k in range(vehicle_number):
                    if qimk[i][m][k].x>0.0001:
                        fenzi+=him[i][m]*qimk[i][m][k].x
    for k in range(vehicle_number):
        fenzi+=bk[k]*yik[0][k].x
    if fenmu != 0:
        final_Z1=fenzi/fenmu
    else:
        final_Z1=0
else:
    final_Z1 = 0
...
for m in range(product_kind):
    for i in range(retailer_number):
        check_obj+=-beta_check * him[i][m]*(-dim_real[i][m])
        for j in range(retailer_number):
            if i != j:
                check_obj.addTerms(beta_check * him[i][m], wijm_check[i][j][m])
                check_obj.addTerms(-beta_check * him[i][m], wijm_check[j][i][m])
